...
int main(int argc, char **argv) {
    ...
    auto V_linear = gen_row_linear(d_model, d_model, seq_len, sparsity, nhead);
    auto O_linear = gen_tile_linear(d_model, d_model, seq_len, sparsity);
    auto MLP1_linear =
        gen_tile_linear(d_model, d_model * 4, seq_len, sparsity, true);
    auto MLP2_linear = gen_tile_linear(d_model * 4, d_model, seq_len, sparsity);
